...
irods_check_flatness = dag.task(task.irods_check_flatness)
...
ckan_create_package = dag.task(task.ckan_create_package)
irods_update_repository_info = dag.task(task.irods_update_repository_info)
irods_remove_ownership = dag.task(task.irods_remove_ownership)

irods_unlock_collection = dag.task(task.irods_unlock_collection, trigger_rule='one_failed')
